...
local function addElement(upvalueLog, upvalue, index, value, temporary)
    ...
    elementLog.Index.Label.TextColor3 = oh.Constants.Syntax[elementIndexType] or Color3.fromRGB(255, 255, 255)
    elementLog.Index.Icon.Image = oh.Constants.Types[elementIndexType] or ""
    elementLog.Value.Label.TextColor3 = oh.Constants.Syntax[elementValueType] or Color3.fromRGB(255, 255, 255)
    elementLog.Value.Icon.Image = oh.Constants.Types[elementValueType] or ""
...
local function addUpvalue(upvalue, temporary)
    ...
    upvalueLog.Index.Text = tostring(index)
    upvalueLog.Value.TextColor3 = oh.Constants.Syntax[valueType] or Color3.fromRGB(255, 255, 255)
    upvalueLog.Icon.Image = oh.Constants.Types[valueType] or ""
...
local function updateUpvalue(closureLog, upvalue)
    if not closureLog or not upvalue then
        return
    end

    local upvalueLog = closureLog.Instance.Upvalues:FindFirstChild(
        tostring(upvalue.Index)
    )
    ...
    if not closure or type(closure.Data) ~= "function" then
        return
    end
    ...
    if valueType == "function" then
        ...
        upvalueLog.Value.Text =
            (closureName == "" and "Unnamed function")
            or closureName
    ...
    elseif valueType == "table" and upvalue.Scanned then
        ...
        if success then
            upvalueLog.Value.Text = text
        else
            upvalueLog.Value.Text = tostring(newValue)
        end
    end

    upvalueLog.Value.TextColor3 =
        oh.Constants.Syntax[valueType]
        or Color3.fromRGB(255, 255, 255)

    upvalueLog.Icon.Image =
        oh.Constants.Types[valueType]
        or ""
...
function Log.new(closure)
    ...
    instance:FindFirstChild("Name").Text = (closure.Name ~= "" and closure.Name) or "Unnamed Closure"
...
function Log.update(log)
    for _i, upvalue in pairs(log.Closure.Upvalues) do
        pcall(updateUpvalue, log, upvalue)
    end
...
local function addUpvalues()
    ...
    if query:gsub(' ', '') ~= '' then
        ...
        local results = Methods.Scan(query, deepSearchFlag)
        for _i, closure in pairs(results) do
...
local function generateScript(elementIndex) 
    ...
    local success, constantsList = pcall(getConstants, closureData)
    if success and type(constantsList) == "table" then
        for idx, constant in pairs(constantsList) do
...
viewUpvaluesContext:SetCallback(function()
    if selectedLog then
        ...
        if temporaryUpvalues then
            ...
            local uvSuccess, uvs = pcall(getUpvalues, closure.Data)
            if uvSuccess and type(uvs) == "table" then
...
getScriptContext:SetCallback(function()
    if selectedLog then
        local success, env = pcall(getfenv, selectedLog.Closure.Data)
...
viewElementsContext:SetCallback(function()
    ...
    if temporaryElements then
        for index, _v in pairs(temporaryElements) do
            local elementLog = selectedUpvalueLog.Elements:FindFirstChild(toString(index))
            if elementLog then
        ...
        local scanned = selectedUpvalue.Scanned or {}
        temporaryElements = {}

        if type(selectedUpvalue.Value) == "table" then
    ...
    if selectedUpvalueLog.Parent and selectedUpvalueLog.Parent.Parent then
...
local function changeUpvalue()
    if selectedUpvalue then
        ...
        indexNumber.Text = tostring(index)
...
changeElementContext:SetCallback(function()
    if selectedUpvalue and selectedElement then
        ...
        indexLabel.TextColor3 = oh.Constants.Syntax[indexType] or Color3.fromRGB(255, 255, 255)
